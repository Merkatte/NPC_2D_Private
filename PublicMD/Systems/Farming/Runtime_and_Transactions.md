...
농경지 배치별 current crop, phase, progress와 수확 입고 transaction을 소유한다.
...
| 경로 | 한 줄 책임 |
|---|---|
| `Assets/Scripts/System/Farming/FarmWorkSite.cs` | 농장 runtime crop·progress·phase와 수확 transaction 및 상태 알림 |
...
- `FarmWorkSite`에는 `SeededRandomSource`와 `IInventory` 구현 source가 필요하다.
