...
- 플레이어는 농경지에 어떤 씨앗을 심을지 선택한다.
- 농부 NPC는 씨앗 종류나 결과물을 판단하지 않고 배정된 농경지에서 기존 업무를 수행한다.
- 농경지는 선택된 작물 정의를 소유하며 성장 요구치, 수확 요구치, 결과물과 수확량을 결정한다.
- 서로 다른 작물은 결과물뿐 아니라 필요한 작업량과 기대 수확량이 달라 생산 선택에 의미를 만들어야 한다.
- 성장 단계와 수확 완료는 sprite와 animation으로 보여주고, 수치 UI와 실제 runtime 상태가 일치해야 한다.
- 씨앗 선택은 매 작업마다 반복하는 명령이 아니라 생산 cycle을 설정하는 플레이어의 관리 결정이다.
- 최종 수확이 완료되면 농경지는 current crop을 비우고 다음 cycle 전에 다시 선택을 요구한다. 수확 소멸 연출은 이 gameplay transaction을 지연시키지 않는다.
...
- 주민이 업무를 수행하지 못하는 이유
- 핵심 자원의 현재 수량과 증감
- 농경지의 현재 작물, 성장 단계, 진행도와 다음에 선택 가능한 씨앗
- 모집 비용과 배치 가능 여부
- 다음 위험 또는 현재 목표
...
| 작물별 결과물·요구치·수확량과 sprite·animation 참조 | ScriptableObject | 첫 구현에서 수치와 Unity 에셋 연결을 하나의 crop definition으로 검증하기 쉽다. |
