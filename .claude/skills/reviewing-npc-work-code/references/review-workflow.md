...
- `PublicMD/ProjectStructure.md`, then the `PublicMD/Systems/` document(s) its routing table points to for the code under review
...
- selector/action/destination ownership rules (decision stays out of action; action does not pick the next long-term goal)
- `WorkerNPC` remaining queue consumption and common runtime only, not role priority or action internals
- `ActionPool` rent/return ownership and whether pooled actions are fully cleared before reuse
