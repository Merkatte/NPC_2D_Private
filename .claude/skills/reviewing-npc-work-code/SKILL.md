...
description: Audits the NPC_Work_2D Unity worker/NPC codebase for code convention compliance, responsibility boundaries, dependency direction, magic numbers, dead code, Unity serialized-reference issues, and architecture drift. Use when asked to review code quality, run a lead-programmer style code audit, update PublicMD/Status/Code_Evaluation_Result.md, or verify that worker actions/selectors/providers/data scripts follow PublicMD/CodeConvention.md and PublicMD/ProjectStructure.md.
---
...
2. Read `PublicMD/ProjectStructure.md` and follow its routing table to the relevant `PublicMD/Systems/` document(s) for the code under review.
3. Read `references/review-workflow.md`.
4. Inspect all relevant `.cs` files under `Assets/Scripts`.
5. Check scene/prefab serialized references when findings involve Unity wiring, selector setup, destinations, providers, or managers.
