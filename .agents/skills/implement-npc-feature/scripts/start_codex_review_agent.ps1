...
$finalPath = Join-Path $resolvedRoot 'PublicMD\Status\Code_Evaluation_Result.md'
...
When invoked:
1. Read the reviewing-npc-work-code skill and PublicMD/ProjectStructure.md.
2. Use the changed-file list and the ProjectStructure routing table to select only the PublicMD/Systems leaf documents that own or consume those files. If a feature is a folder, use its README only to choose the relevant leaf; do not read every sibling leaf.
3. Read PublicMD/CodeConvention.md for C# changes. Read PublicMD/ARCHITECTURE.md only when the change moves ownership or dependencies across systems.
4. Run git status, git diff, and targeted read/search commands.
5. Focus on changed files and inspect directly affected dependency surfaces.
6. Verify every claim from repository evidence.
...
Use the reviewing-npc-work-code audit methodology. Your sandbox is read-only: do not attempt to modify any file. Return only the complete Markdown body intended for PublicMD/Status/Code_Evaluation_Result.md as the final response. The launcher owns writing that final response to the report file.
"@
